...
The trained model achieves an average Dice coefficient of **0.7414** and an average Intersection-over-Union (IoU) of **0.5902** on the validation set. To convert segmentation outputs into nuclei counts, predicted probability maps are thresholded and analysed using connected-components labelling with a minimum-area filter (`MIN_AREA = 5`). A threshold sensitivity study over the range 0.2–0.9 indicates that thresholds of 0.6–0.7 yield the most accurate counts; the value 0.7 is selected as it minimises the Mean Absolute Error (MAE) of the count on validation, achieving an MAE of **70.75** and a Mean Squared Error (MSE) of **6849.75**. A watershed post-processing variant was evaluated and rejected, as it degraded counting performance (MAE 111.76, MSE 23920.89). On the held-out 4-image test split, average Dice was **0.3787** with MAE **214.0**; this is dominated by a single failure tile and is interpreted as a *trend indicator* rather than a population estimate (Section 9.4).

The system is delivered as a productised stack: a FastAPI service with persistent history (SQLite by default, Postgres opt-in), structured JSON logging with per-request IDs, per-IP rate limiting, ETag-based response caching, and 20 passing pytest integration tests; a React 18 + Vite + Tailwind dashboard with a router-driven navigation, an error boundary, a one-click demo mode, dark/light themes and a mobile-responsive layout; and a research-only Streamlit dashboard that inspects pre-computed pipeline outputs. The reported numbers should be interpreted as a baseline academic result, not clinical-level performance.
...
### 9.4 Held-out test-set results

The 4-image test split was kept entirely unseen during training and threshold tuning, and was evaluated only after the final operating point was fixed. Re-running the pipeline on the test split with the locked configuration (τ = 0.7, `MIN_AREA = 5`) produced the following per-image and aggregate results, generated by `src/eval_test_set.py`:

| Image | Dice | IoU | Pred. count | GT count | Abs. error |
|---|---:|---:|---:|---:|---:|
| TCGA-B0-5711-01Z-00-DX1 | 0.7603 | 0.6133 | 254 | 342 | 88 |
| TCGA-G9-6362-01Z-00-DX1 | 0.2195 | 0.1233 | 178 | 472 | 294 |
| TCGA-E2-A14V-01Z-00-DX1 | 0.5325 | 0.3629 | 257 | 378 | 121 |
| TCGA-G9-6363-01Z-00-DX1 | 0.0025 | 0.0013 |   1 | 354 | 353 |
| **Average** | **0.3787** | **0.2752** | — | — | **214.00** |

The aggregate test-set Dice (0.3787) is materially below the validation Dice (0.7414). The drop is dominated by `TCGA-G9-6363-01Z-00-DX1`, on which the model fails almost completely (Dice ≈ 0). The remaining three test images give Dice scores in the 0.22–0.76 range, consistent with the validation distribution. We treat this as evidence of two phenomena that an honest test-set evaluation is intended to surface:

- **High variance over a 4-image split.** A single failure case shifts the aggregate by roughly 0.18 Dice. Reported test averages should be read as *trend indicators*, not estimates of population performance.
- **Limited training generalisation.** With only 29 training images, slide-level domain shift between training and test patients is sufficient to break the model on outlier tiles. This is the principal limitation called out in Section 13 and is the main motivation for the future-work directions in Section 15.

The full per-image CSV, predicted masks, and overlays are written to `outputs/test_set_eval/` for inspection.
